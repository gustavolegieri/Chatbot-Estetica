...
const flow = [
  ...
  { id: "FINALIZED", label: "Finalização", detail: "Veículo saiu", icon: Sparkles },
];
...
function statusLabel(status: string) {
  return ({ PENDING: "Pendente", CONFIRMED: "Confirmado", IN_PROGRESS: "Em lavagem", COMPLETED: "Finalizado" } as Record<string, string>)[status] || status;
}
...
export function GateVisionPanel() {
  ...
  return <div className="pb-24 lg:pb-8">
    <AdminHeader
      title="Portão IA"
      description="A webcam inicia a lavagem na entrada e finaliza o atendimento na saída."
    ...
    <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {[
        ...
        { label: "Saídas em 7 dias", value: data.metrics.exitsWeek, detail: "Atendimentos finalizados", icon: ArrowUp, tone: "green" },
    ...
    <Panel className="mb-6">
      <PanelTitle icon={ScanLine} title="Fluxo oficial da garagem" subtitle="A IA exige que o mesmo veículo atravesse as duas linhas virtuais, reduzindo disparos por pessoas e sombras." />
    ...
    <div className="grid gap-6 xl:grid-cols-[1.02fr_.98fr]">
      <Panel><PanelTitle icon={Clock3} title="Eventos do portão" subtitle="Histórico auditável com placa, associação, confiança e leituras descartadas." /><div className="max-h-[520px] space-y-3 overflow-y-auto border-l border-brand-400/15 pl-4">{data.timeline.map((item: RecordData) => <article key={item.id} className="relative"><span className={cn("absolute -left-[20.5px] top-4 h-2 w-2 rounded-full", item.type === "ENTER" ? "bg-sky-400" : item.type === "EXIT" ? "bg-emerald-400" : "bg-slate-600")} /><div className="rounded-2xl border border-white/[0.05] bg-black/10 p-3"><div className="flex items-center justify-between gap-3"><div className="flex flex-wrap items-center gap-2"><strong className="text-xs text-slate-200">{item.label}</strong><span className={cn("rounded-full px-2 py-0.5 text-[8px] font-bold", item.matched ? "bg-emerald-400/10 text-emerald-300" : item.type === "IGNORED" ? "bg-slate-700/50 text-slate-500" : "bg-amber-400/10 text-amber-300")}>{item.matched ? "agendamento associado" : item.type === "IGNORED" ? "descartado" : "sem associação"}</span>{item.plate && <span className="rounded border border-brand-300/15 bg-brand-300/[0.06] px-1.5 py-0.5 font-mono text-[8px] font-bold tracking-wider text-brand-200">{item.plate} · {Math.round((item.plateConfidence || 0) * 100)}%</span>}</div><span className="shrink-0 text-[8px] text-slate-600">{formatDate(item.at)}</span></div><p className="mt-2 text-[10px] text-slate-500">{item.clientName ? `${item.clientName}${item.vehicle ? ` · ${item.vehicle}` : ""}` : item.reason || "Detecção automática do veículo"}</p>{item.confidence > 0 && <div className="mt-2 flex items-center gap-2"><div className="h-1 flex-1 rounded-full bg-white/[0.05]"><div className="h-full rounded-full bg-brand-300" style={{ width: `${Math.round(item.confidence * 100)}%` }} /></div><span className="text-[8px] text-brand-300">veículo {Math.round(item.confidence * 100)}%</span></div>}</div></article>)}{!data.timeline.length && <p className="py-12 text-center text-xs text-slate-600">Os eventos aparecerão após conectar a webcam.</p>}</div></Panel>
      <Panel><PanelTitle icon={CalendarClock} title="Agenda usada pela automação" subtitle="A entrada só é associada quando a placa lida coincide com a placa cadastrada." /><div className="space-y-2">{data.appointments.map((item: RecordData) => <article key={item.id} className={cn("flex items-center gap-3 rounded-2xl border p-3", item.isCurrent ? "border-brand-400/25 bg-brand-400/[0.06]" : "border-white/[0.055] bg-black/10")}><span className="grid h-10 w-12 shrink-0 place-items-center rounded-xl bg-brand-400/10 font-mono text-xs font-bold text-brand-200">{item.time}</span><div className="min-w-0 flex-1"><strong className="block truncate text-xs text-slate-200">{item.clientName}</strong><p className="mt-1 truncate text-[9px] text-slate-600">{item.vehicle || "Veículo não informado"} · {item.service}</p></div>{item.plate ? <span className="rounded-lg bg-white/[0.045] px-2 py-1 font-mono text-[9px] font-bold tracking-wider text-slate-300">{item.plate}</span> : <span className="text-[8px] font-bold text-amber-300">sem placa</span>}<span className={cn("rounded-full px-2 py-1 text-[8px] font-bold", item.status === "IN_PROGRESS" ? "bg-sky-400/10 text-sky-300" : item.status === "COMPLETED" ? "bg-emerald-400/10 text-emerald-300" : "bg-white/[0.05] text-slate-500")}>{statusLabel(item.status)}</span></article>)}{!data.appointments.length && <p className="rounded-2xl border border-dashed border-white/10 p-8 text-center text-xs text-slate-600">Nenhum atendimento na agenda de hoje.</p>}</div></Panel>
    </div>

    <div className="mt-6 grid gap-6 xl:grid-cols-2">
      <Panel><PanelTitle icon={Camera} title="Como instalar a câmera" subtitle="Não existe microcontrolador: apenas a webcam conectada ao computador da estética." /><div className="relative overflow-hidden rounded-2xl border border-white/[0.06] bg-[#090a0b] p-5"><div className="grid h-44 grid-rows-[1fr_auto_1fr] text-center"><div className="flex items-center justify-center text-[10px] font-bold uppercase tracking-[.2em] text-slate-600">Rua / lado de fora</div><div><div className="h-px bg-amber-400/60" /><div className="my-5 flex items-center justify-center gap-3"><div className="h-px flex-1 bg-white/[0.06]" /><span className="rounded-xl border border-white/[0.08] bg-white/[0.03] px-4 py-2 text-[10px] font-bold text-slate-400">PORTÃO</span><CarFront className="h-6 w-6 text-brand-300" /><div className="h-px flex-1 bg-white/[0.06]" /></div><div className="h-px bg-emerald-400/60" /></div><div className="flex items-center justify-center text-[10px] font-bold uppercase tracking-[.2em] text-slate-600">Garagem / lado de dentro</div></div><div className="absolute left-5 top-[71px] rounded bg-amber-400/10 px-2 py-1 text-[8px] text-amber-300">linha FORA</div><div className="absolute bottom-[67px] left-5 rounded bg-emerald-400/10 px-2 py-1 text-[8px] text-emerald-300">linha DENTRO</div></div><div className={cn("mt-4 rounded-2xl border p-3 text-[10px] leading-5", data.configuration.tokenConfigured ? "border-emerald-400/15 bg-emerald-400/[0.04] text-emerald-200" : "border-amber-400/15 bg-amber-400/[0.04] text-amber-100")}>{data.configuration.tokenConfigured ? <><CheckCircle2 className="mr-2 inline h-4 w-4" />Token da câmera configurado no servidor.</> : <>Configure <strong>GATE_VISION_DEVICE_TOKEN</strong> na Vercel antes de ligar o agente local.</>}</div></Panel>
