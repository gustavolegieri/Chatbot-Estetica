...
export async function startCampaignProcessing(campaignId: string, opts: ProcessorOptions = {}) {
  ...
  try {
    ...
    console.error(`[CampaignProcessor] Erro ao finalizar campanha ${campaignId}:`, error);
  }

  globalAny.__campaignProcessors.delete(campaignId);
  } catch (error) {
    console.error(`[CampaignProcessor] Erro ao iniciar/processar campanha ${campaignId}:`, error);
    globalAny.__campaignProcessors.delete(campaignId);
    throw error; // Re-lançar para quem chamou poder tratar
  }
}
...
export async function pauseCampaign(campaignId: string) {
  try {
    ...
    console.error(`[CampaignProcessor] Erro ao pausar campanha ${campaignId}:`, error);
    throw error;
  }
}
...
export async function resumeCampaign(campaignId: string, opts: ProcessorOptions = {}) {
  try {
    ...
    console.error(`[CampaignProcessor] Erro ao retomar campanha ${campaignId}:`, error);
    throw error;
  }
}
