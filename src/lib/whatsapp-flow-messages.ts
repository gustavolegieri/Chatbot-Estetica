...
export function serviceDetail(item: CatalogItem, prompts?: PromptMap, detailOverride?: string | null) {
  ...
  const serviceTexts: Record<string, string[]> = {
    lavagem_simples: [
      `💧 *Lavagem Simples*`,
      ``,
      `Lavagem Simples é a lavagem mais básica que oferecemos. Ideal para manutenção regular do visual.`,
      ``,
      `O que inclui:`,
      `• Ducha e secagem total`,
      `• Lavagem manual`,
      `• Limpeza de rodas e pneus`,
      `• Aplicação de pretinhos nos pneus`,
    ...
    lavagem_detalhada: [
      ...
      `• Aplicação de cera em pasta`,
      `• Limpeza de pedais e trilhos`,
      `• Tratamento nos vidros`,
      ``,
